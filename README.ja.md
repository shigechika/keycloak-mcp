...
| `get_ip_activity` | 送信元 IP 1件の全活動を漏れなく調査：成功/失敗件数、対象ユーザ・クライアント、タイムライン。構造化 JSON を返す |
...
`spray_check` は「いまから遡る移動窓」なので、パトロールには合うが日ごとに保存できない。定期ジョブから暦日 1 日分を同じ判定で集計するには:
...
- 中身は `spray_check` と同じ形で、外部 IP を全件（`min_report_users=1`）載せ、加えて `external_totals`（外部 IP ごとの LOGIN / LOGIN_ERROR 件数。ユーザー名の無い失敗も数える）・`fetch_complete`・`resolve_complete`・`coverage`（`first_event`・`last_event`・`tail_gap_seconds`）・`schema`・`keycloak_mcp_version`・`sites_ini_sha256` を持つ。両方の `*_complete` が true の日だけを確定値として扱う

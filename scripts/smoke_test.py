#!/usr/bin/env python3
"""Exercise every registered tool against the live realm and report what is broken.
...
Read-only by construction: every state-changing tool is skipped by name in
``smoke_probes.py``, and the report prints only tool names, statuses and row
counts — never payloads, which would otherwise carry realm data into logs.

Usage:
    # In-process: imports the server, uses the configured service account.
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        results = await run_probes(
            ...
            # Realm data must not reach the report: KeyCloak errors quote the
            # username or group they were asked about. --traceback still shows
            # the full text on the operator's own terminal when debugging.
            redact_details=True,
        )
    finally:
...
def main() -> None:
    ...
    # Serial by default: the realm is shared production infrastructure, and a
    # burst of admin API calls is a worse neighbour than a slow smoke test.
